...
- `src/app/` — App Router routes.
  ...
  - Transfers between the household's own accounts: `transfers` table (one row per pair; `out_transaction_id` is the `> 0` leg, `in_transaction_id` the `< 0` leg, DB triggers keep that invariant and drop a pair whose leg no longer nets to zero). Pure matcher `src/lib/transfer-match.ts`, I/O `src/lib/transfer-detect.ts` (prefers the service client so both legs are visible), loaders `src/lib/transfer-legs.ts`. Detection runs inside `applyRulesToTransactions`, so every ingest path gets it even with zero rules; `src/lib/cron/transfer-backfill.ts` runs one whole-ledger pass per household from the daily cron (`households.transfers_backfilled_at`). Reports skip legs; balances, net worth and contributions do not. "Not a transfer" sets `transactions.transfer_ignored`.
  ...
  - `api/cron/daily/route.ts` — the single Vercel Cron entry (`vercel.json`, Hobby allows daily only): Plaid safety-net sweep + settlement auto-close + transfer backfill. Bearer `CRON_SECRET`; every job idempotent.
  - `api/plaid/webhook/route.ts` — signed Plaid webhooks, sync enqueued via `after()`.
- Per-member privacy: RLS is private-by-default (`account_visible` / `tx_visible` / `tx_editable` helpers, migrations `20260826000004` + `20260826000007`). A login sees exactly three kinds of money: its own member's accounts and payments, joint accounts (`ownership = 'shared'`), and crossover transactions another member paid that it holds a `transaction_shares` row for. Members without a login are visible to nobody. Crossover rows are read-only; the UI hides write affordances for them (`isTxEditable` / `classifyTx` in `src/lib/tx-scope.ts` mirror the SQL). The payer and account owner are always stamped from `getHouseholdContext().memberId`, never picked from a list. Transfers are the one cross-boundary link: the matcher runs with the service role so legs on two members' private accounts still pair, but a login sees the counterpart's name only when it can already see that account (a `transfers` row is visible to whoever sees either leg).
...
Email: every email, Supabase Auth's included, goes out through the Resend HTTP API via `src/lib/email/send.ts` (`RESEND_API_KEY`, `EMAIL_FROM`). Supabase Auth reaches it through the "Send Email" auth hook (dashboard → Authentication → Auth Hooks → `https://<site>/api/auth/send-email`, secret in `SEND_EMAIL_HOOK_SECRET`; handler `src/app/api/auth/send-email/route.ts`, rendering in `src/lib/auth-email.ts`, signature check in `src/lib/standard-webhooks.ts`). Links in those emails go straight to `/auth/confirm?token_hash=…&type=…`. The dashboard SMTP settings (`smtp.resend.com:465`, user `resend`) and pasted templates are the fallback if the hook is disabled - mail sent over that relay was landing in Gmail spam while API-sent mail reached the inbox, which is why the hook exists. Production uses `Maple <maple@send.foothillspcrepair.ca>` (a Resend-verified domain) for both the Supabase SMTP sender and `EMAIL_FROM`; the code default `Maple <onboarding@resend.dev>` is Resend's sandbox sender and only delivers to the Resend account owner, and Supabase surfaces that as `Error sending confirmation email` with no user created. To probe delivery use Resend's test inbox `delivered@resend.dev` - Resend rejects `example.com` recipients outright. All email HTML comes from one layout, `src/lib/email/layout.ts`; content lives in `src/lib/email/templates.ts`. `node scripts/build-email-templates.ts` regenerates `docs/email-templates/*.html` for pasting into the Supabase Templates tab - never hand-edit those files.
