...
//   2. Settlement period auto-close (wired in Phase 4).
...
export async function GET(request: NextRequest) {
  ...
  const plaid = await runPlaidSweep(service, { budgetMs: 240_000, perItemMs: 45_000 })
  console.log('[cron/daily] plaid sweep', { ...plaid, items: undefined })

  return NextResponse.json({ ok: true, ms: Date.now() - started, plaid })
}
