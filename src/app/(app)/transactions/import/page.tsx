...
export default async function ImportPage() {
  ...
  return (
    <div className="flex flex-col gap-6 pb-10">
      ...
      <section className="flex flex-col gap-4 rounded-lg border border-hair bg-leaf-tint p-5 md:flex-row md:items-center md:justify-between md:gap-6 md:p-6">
        ...
        <Link
          ...
          className="inline-flex min-h-[44px] shrink-0 items-center gap-2 self-start rounded-full bg-leaf px-5 py-3 text-[13.5px] font-semibold text-paper shadow-[var(--shadow-card)] transition-transform active:scale-[0.98] md:self-auto"
        >
      ...
      {!hasAccounts ? (
        <section className="rounded-lg border border-dashed border-hair bg-paper p-6 text-[13.5px] text-ink-2">
          Add at least one account first.{' '}
          <Link href="/accounts" className="font-semibold text-ink underline-offset-2 hover:underline">
            Go to accounts
          </Link>
          .
        </section>
...
function Hint({ tag, items }: { tag: string; items: string[] }) {
  return (
    <div className="rounded-md border border-hair bg-paper p-4">
      <div className="inline-block rounded-full bg-leaf-soft px-2 py-0.5 text-[10px] font-semibold uppercase tracking-[0.06em] text-leaf">
        {tag}
      </div>
